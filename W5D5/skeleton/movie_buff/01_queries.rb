...
def harrison_ford
  ...
  # Movie.joins(:actors, :castings).where(acto)s: {name: "Harrison Ford" } .where(_not) castings: { ord = 1 }
  Movie.select(:id, :title).joins(:actors, :castings).where(actors: {name: "Harrison Ford" } ).where.not( castings: { ord: 1 }).group( :id)
end
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:actors).group(:id).order("COUNT(*) DESC").limit(3)
end
